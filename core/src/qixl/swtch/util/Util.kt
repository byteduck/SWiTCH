...
object Util {
    ...
    fun slideCamera(cam: OrthographicCamera, dir: Direction, manager: TweenManager, widthMeasurement: Float, heightMeasurement: Float, ease: TweenEquation, delay: Float, callback: ((Int, BaseTween<*>) -> Unit)?): Tween {
        ...
        when (dir) {
            Direction.RIGHT -> t.target(-widthMeasurement / 2, heightMeasurement / 2, 0f)
            Direction.LEFT -> t.target(widthMeasurement * 1.5f, heightMeasurement / 2, 0f)
            Direction.CENTER -> t.target(widthMeasurement / 2, heightMeasurement / 2, 0f)
